...
APP_INSTALL_WITH_CMD = f"docker run -itd -v /opt/k2root:/opt/k2root -e K2_OPTS=' -javaagent:/opt/k2root/k2root/collectors/k2-java-agent.jar' -e K2_GROUP_NAME={K2.K2_GROUP_NAME} -e JAVA_TOOL_OPTIONS='-Xms4g -Xmx4g' -p 8080:8080 --name {APP_CONTAINER_NAME} k2cyber/test_application:{APP_CONTAINER_NAME}"
APP_INSTALL_WITHOUT_CMD = f"docker run -itd -e JAVA_TOOL_OPTIONS='-Xms4g -Xmx4g' -p 8080:8080 --name {APP_CONTAINER_NAME} k2cyber/test_application:{APP_CONTAINER_NAME}"
